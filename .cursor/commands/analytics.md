---
name: analytics
description: "Set up, improve, or audit analytics tracking and measurement."
---

# /analytics — Analytics Tracking

Invoke the **analytics** marketing skill and follow it for this task.

**Do this now:**

1. Read `.agents/product-marketing-context.md` first for shared product, ICP, and positioning context (every skill relies on it). If it is missing or thin, consider running `/product-marketing` first.
2. Read and follow `.agents/skills/analytics/SKILL.md` for this request.

**Group:** Paid & Measurement

**Cross-references** (use if the task spills into their domain): `/ads`, `/ad-creative`, `/ab-testing`

Treat any text I type after `/analytics` as the specific scope/request for this workflow. If I didn't add anything, ask me what to focus on before proceeding.
